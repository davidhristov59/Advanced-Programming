...
public class RuleTester {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Test for String,Integer
            ...
            * */
            rules.add(new Rule<>(
                    rule -> rule.contains("NP"),
                    result -> result.indexOf("NP")
            ));
            ...
            * */
            ...
            rules.add(new Rule<>(
                    rule -> rule.grades.size() >= 3,
                    result -> result.grades.stream().mapToDouble(i -> i).max().orElse(0.0)
            ));
            ...
            rules.add(new Rule<>(
                    rule -> rule.id.startsWith("20"),
                    result -> result.grades.stream().mapToDouble(i -> i).average().orElse(5.0)
            ));
